...
- reduced Spotify refresh request volume by reusing `/me/playlists` artwork and `/me/albums` album objects instead of issuing per-playlist cover and per-album detail requests; unchanged libraries now require only the paginated profile/library/list calls plus changed-playlist and oversized-album pagination
